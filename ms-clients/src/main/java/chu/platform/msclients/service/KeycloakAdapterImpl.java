package chu.platform.msclients.service;//package chu.platform.msclient.service;

import chu.platform.msclients.dto.AuthenticationResponse;
import chu.platform.msclients.dto.UserDTO;
import chu.platform.msclients.service.iservices.IKeycloakAdapter;
...
public class KeycloakAdapterImpl implements IKeycloakAdapter
{
    ...
    public boolean checkIfUsernameOrEmailExist(String value) {
        try{
            ...
            return usersResource.search(value).isEmpty();
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
        ...
        try {
            ...
            if(response.getStatusInfo().getFamily()==Response.Status.Family.SUCCESSFUL){
                ...
                getAdminKeycloak().realm(realm).users().get(clientId).roles().realmLevel().remove(Collections.singletonList(roleRepresentationList.stream().filter(
                        roleRepresentation -> roleRepresentation.getName().equals("default-roles-platform-chu-stage-realm")
                ).findFirst().get()));
                getAdminKeycloak().realm(realm).users().get(clientId).roles().realmLevel().add(
                        roleRepresentationList.stream().filter(
                                roleRepresentation -> roleRepresentation.getName().equals("Stagiaire")
